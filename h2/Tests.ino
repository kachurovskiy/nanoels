...
test(checkPlusMinusButtons) {
  ...
  assertEqual(10, tmmpr);
  assertEqual(10, tmmprPrevious);
...
test(moveLeftIsOffNegativeTmmpr) {
  isOn = false;
  tmmpr = -500;
  pos = 0L;
  spindlePos = 0L;
...
test(moveRightIsOffNegativeTmmpr) {
  isOn = false;
  tmmpr = -500;
  pos = 0L;
  spindlePos = 0L;
